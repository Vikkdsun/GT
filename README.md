...
#### 当输入GT的复位一直拉高，QPLL lock的反而比outclk慢，而且由于一直拉高，不会初始化成功

![image](https://github.com/Vikkdsun/GT/assets/114153159/e9d289e8-ed20-4318-a008-eba6f8945e11)

#### 上图才是正常时序，也就是i_tx_reset一直为0（不复位），✨***由于QPLL的reset和GT的tx_reset我们写的时候绑定了关系***✨，所以当i_tx_reset为0，产生一个QPLL的reset(图中有两个reset 上面_i是ip输出的 下面的_t是做了或操作 送到COMMON的真正reset)，过了这个reset后，QPLL先lock，然后才有outclk lock 然后userclk2，最后初始化成功

![image](https://github.com/Vikkdsun/GT/assets/114153159/0125115e-92a8-4e22-8529-c8438a652d13)

#### 但是当我们在i_tx_reset（图中最上面的信号）一直为1（没有clk2时）时，不会产生QPLL的reset，很快产生outclk以及outclk2，虽然QPLL很快lock，但是还是慢于outclk，这时错误的，随着clk2的产生，i_tx_reset又变0了，然后QPLL重新reset了，又重新lock了outclk和userclk2，这回顺序才对

#### 从中我们知道，GT输入的TX_reset为高不会限制QPLL和outclk的lock，但是变0的话会有一个QPLL的reset，然后重新生成QPLL lock、然后outclk lock。

#### 但是tx_reset为高会限制GT的初始化，虽然产生了QPLL和outclk，但是GT还没初始化成功，只有QPLL成功lock，然后outclk成功lock一段时间后，才有初始化成功，也就是图中第二个金色信号

>总结 1 输入GT的复位可以一直为0，这样会让初始化走下去；2 QPLL必须有一次复位，这样会让QPLL先复位一下再重新生成lock，生成之后才有outclk lock，这样正常时序
>2 输入GT的复位可以先1后0，只要有1，初始化就可以进行；先1导致QPLL没有reset,outclk生成lock，并且快于QPLL，但是复位变0后，导致QPLL产生一次复位，QPLL重新生成lock，outclk就消失了，直到QPLL成功生成后，才有outclk lock，这回变成正常时序了。
>但是输入GT的复位不能一直为1，首先不会产生QPLL复位，导致时序不对，其次不会初始化成功。

>也就是必须现有QPLL lock，然后有outclk和useclk2 lock，然后才有GT的工作状态。
